...
public class ItemServiceImpl implements ItemService {
    ...
    @Override
    public ItemDto createItem(ItemSaveDto itemSaveDto, Long userId) {
        if (itemSaveDto.getAvailable() == null) {
            throw new ValidationException("Available must not be blank or null");
        }
        ItemRequest request = null;
        if (itemSaveDto.getRequestId() != null) {
            request = itemRequestRepository.findById(itemSaveDto.getRequestId()).orElseThrow(()
                    -> new NotFoundException("Request  " + itemSaveDto.getRequestId() + " is not found"));
        }

        User owner = userRepository.findById(userId)
                .orElseThrow(() -> new NotFoundException("User not found"));
        Item newItem = itemMapper.toItem(itemSaveDto, null, request, owner);
